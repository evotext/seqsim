...
@pytest.mark.parametrize(
    ...
    [
        ...
        [(1, 2, 3), [1, 2, 3]] + [0.0, 0.0],
        [(1, 2, 3, 4, 5), (1, 2, 4, 3, 6, 7)] + [0.272727, 1e-6],
        [(1, 2, 3), ["a", "b", "c", "d"]] + [1.0, 0.0],
    ],
)
def test_sorensen_distance(seq_x, seq_y, expected, tol):
    # Test hard-coded expected value
    assert token.sorensen_dist(seq_x, seq_y) == pytest.approx(expected, abs=tol)

    # Test symmetry
    assert token.sorensen_dist(seq_x, seq_y) == token.sorensen_dist(seq_y, seq_x)

    # Test triangle-inequality
    seq_z = [element for element in seq_x] + [element for element in seq_y]
    dist_xz = token.sorensen_dist(seq_x, seq_z)
    dist_xy = token.sorensen_dist(seq_x, seq_y)
    dist_yz = token.sorensen_dist(seq_y, seq_z)
    assert dist_xz <= (dist_xy + dist_yz)
